...
class CtBarber {
  static async getAllBarbers(req, res) {
    try {
      const allBarbers = await pool.query(
        `SELECT u_firstname, u_lastname, bi_id, b_id, b_city, b_status, b_shop_name, b_shop_location FROM barber_info JOIN users ON barber_info.b_id = users.u_id`
      );
  ...
  static async removeAppointmentRequest(req, res) {
    try {
      const { uar_id } = req.params;
      const appointment = await pool.query(
        `SELECT * FROM user_appointment_req WHERE uar_id = '${uar_id}'`
      ...
      res.json("Appointment request removed");
    } catch (err) {
      console.error(err.message);
    }
  }
