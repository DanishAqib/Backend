...
const srCheckIfUserExists = async (u_email, u_password, isBarber) => {
  try {
    ...
    if (
      (userRole === "barber" && isBarber === true) ||
      (userRole === "customer" && isBarber === undefined)
    ) {
      return {
        userRole,
        status: true,
...
const srCheckIfEmailExistsForReset = async (u_email, isBarber) => {
  try {
    const user = await pool.query(
      `SELECT * FROM users WHERE u_email = '${u_email}'`
    );
...
class CtUser {
  static async userlogin(req, res) {
    try {
      const { u_email, u_password, isBarber } = req.body;
      if (!u_email || !u_password) {
        return res.status(400).json("Please provide an email and password");
      }
      const userExists = await srCheckIfUserExists(
        u_email,
        u_password,
        isBarber
      );
  ...
  static async addUser(req, res) {
    try {
      ...
      if (
        ...
        !u_email ||
        !u_password ||
        (u_role === "barber" && (!b_city || !b_shop_name || !b_shop_location))
      ) {
      ...
      if (u_role === "barber" && b_city && b_shop_location)  {
        await pool.query(
          `INSERT INTO barber_info (b_id, b_status, b_city, b_shop_name, b_shop_location) VALUES ('${newUser.rows[0].u_id}', 'available', '${b_city}', '${b_shop_name}', '${b_shop_location}') RETURNING *`
        );
      }
  ...
  static async updateUserPassword(req, res) {
    try {
      ...
      const userExists = await srCheckIfEmailExistsForReset(u_email, isBarber);
      if (!userExists) {
